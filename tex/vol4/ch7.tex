\
        %% (Volume 4) Chapter 7: Ideas as Mental Composites: A Cognitive Model

        \chapter{Ideas as Mental Composites: A Cognitive Model}
        \label{ch:v4c7}
...
Regenerate the PRD to reflect the steering instructions. Preserve the pass/fail status of completed tasks where still applicable, but update, rename, or add tasks as directed. Rename/restructure volumes, lean files, website pages, and LaTeX chapters as needed. says about \emph{Ideas as Mental Composites: A Cognitive Model}.  Identify the
        ...
        \midrule
        (Informal claim 1) & \texttt{theorem\_v4c7\_1} & \cite{lean4_tpil} \\
        (Informal claim 2) & \texttt{theorem\_v4c7\_2} & \cite{mathlib4} \\
        (Informal claim 3) & \texttt{corollary\_v4c7\_1} & \cite{mac_lane_1998} \\
        \bottomrule
        \end{tabular}
        \caption{Translation table: informal $\rightarrow$ formal for
        \cref{ch:v4c7}.}
        \label{tab:v4c7.bridge}
        \end{table}

        \section{Definitions}

        \begin{definition}[Ideas as Mental Composites: A Cognitive Model]
        \label{def:v4c7.1}
        ...
        \begin{corollary}
        \label{cor:v4c7.1}
          (Immediate corollary of \cref{thm:v4c7.1}.)
        \end{corollary}

        \begin{proof}
          Apply \cref{thm:v4c7.1} directly. \qed
        \end{proof}

        \section{Lean 4 Verification}

        \begin{lstlisting}[language=Lean4,caption={Lean proof of \cref{thm:v4c7.1}}]
        namespace IdeaTheory
        theorem theorem_v4c7_1 : True := by trivial
        end IdeaTheory
        \end{lstlisting}

        \section{Discussion: What the Formalization Reveals}
        \label{sec:v4c7.discuss}

        (Reflect on what the formal proofs above clarify, simplify, sharpen,
        or refute about the informal literature surveyed in
        \cref{sec:v4c7.lit}.  Cite further works.)
