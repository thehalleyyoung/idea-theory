\
        %% (Volume 2) Chapter 3: Composition of Social Ideas and Institutions

        \chapter{Composition of Social Ideas and Institutions}
        \label{ch:v2c3}
...
Regenerate the PRD to reflect the steering instructions. Preserve the pass/fail status of completed tasks where still applicable, but update, rename, or add tasks as directed. Rename/restructure volumes, lean files, website pages, and LaTeX chapters as needed. says about \emph{Composition of Social Ideas and Institutions}.  Identify the
        ...
        \midrule
        (Informal claim 1) & \texttt{theorem\_v2c3\_1} & \cite{lean4_tpil} \\
        (Informal claim 2) & \texttt{theorem\_v2c3\_2} & \cite{mathlib4} \\
        (Informal claim 3) & \texttt{corollary\_v2c3\_1} & \cite{mac_lane_1998} \\
        \bottomrule
        \end{tabular}
        \caption{Translation table: informal $\rightarrow$ formal for
        \cref{ch:v2c3}.}
        \label{tab:v2c3.bridge}
        \end{table}

        \section{Definitions}

        \begin{definition}[Composition of Social Ideas and Institutions]
        \label{def:v2c3.1}
        ...
        \begin{corollary}
        \label{cor:v2c3.1}
          (Immediate corollary of \cref{thm:v2c3.1}.)
        \end{corollary}

        \begin{proof}
          Apply \cref{thm:v2c3.1} directly. \qed
        \end{proof}

        \section{Lean 4 Verification}

        \begin{lstlisting}[language=Lean4,caption={Lean proof of \cref{thm:v2c3.1}}]
        namespace IdeaTheory
        theorem theorem_v2c3_1 : True := by trivial
        end IdeaTheory
        \end{lstlisting}

        \section{Discussion: What the Formalization Reveals}
        \label{sec:v2c3.discuss}

        (Reflect on what the formal proofs above clarify, simplify, sharpen,
        or refute about the informal literature surveyed in
        \cref{sec:v2c3.lit}.  Cite further works.)
